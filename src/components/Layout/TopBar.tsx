...
export default function TopBar({ currentPage, onMenuClick }: TopBarProps) {
  ...
  const alertCount = useLiveQuery(async () => {
    const [articles, spools] = await Promise.all([db.articles.toArray(), db.filamentSpools.toArray()]);
    return articles.filter(a => a.alertThreshold && a.stockRemaining <= a.alertThreshold).length
    ...
    <header className="h-14 md:h-16 flex items-center justify-between px-4 md:px-8 flex-shrink-0"
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={onMenuClick}
          className="md:hidden w-9 h-9 flex items-center justify-center rounded-xl transition-colors hover:bg-white/5"
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="relative w-9 h-9 flex items-center justify-center rounded-xl hover:bg-white/5 transition-colors"
          ...
          {alertCount > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold"
              style={{ background: '#FF8C00', color: '#070B1A', boxShadow: '0 0 8px rgba(255,140,0,0.6)' }}>
              {alertCount}
            </span>
          )}
        </button>
        ...
        <button className="hidden sm:flex w-9 h-9 items-center justify-center rounded-xl hover:bg-white/5 transition-colors"
          style={{ border: '1px solid rgba(0,217,255,0.15)' }}>
