...
export default function Dashboard() {
  const { articles, spools, productionHistory, spoolHistory } = useApp();
  ...
  const consumption30d    = spoolHistory.filter(h => new Date(h.date) >= thirtyDaysAgo).reduce((s, h) => s + h.weightUsed, 0);

  const chartData = [...productionHistory].reverse().slice(0, 8).reverse().map(h => ({
    name: h.productName.length > 10 ? h.productName.slice(0, 10) + '…' : h.productName,
    coût: +h.totalCost.toFixed(2), prix: +h.suggestedPrice.toFixed(2), marge: +h.grossProfit.toFixed(2),
  }));
  ...
  const statCards = [
    { label: 'Coût matière total',      value: `${fmt(totalMaterialCost)} €`, icon: DollarSign, color: '#00D9FF', sub: `${productionHistory.length} production(s)` },
    { label: 'Valeur stock composants', value: `${fmt(stockValue)} €`,        icon: Package,    color: '#8B5CF6', sub: `${articles.length} article(s)` },
    { label: 'Produits fabriqués',      value: productionHistory.length.toString(), icon: Cpu, color: '#00FF88', sub: 'dans l\'historique' },
  ...
  return (
    <div className="space-y-6">
      {allAlerts.length > 0 && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
          <div className="rounded-xl px-4 py-3 flex items-start gap-3" style={{ background: 'rgba(255,140,0,0.08)', border: '1px solid rgba(255,140,0,0.3)' }}>
            ...
            <div>
              <div className="text-sm font-semibold" style={{ color: '#FF8C00' }}>Stock faible ({allAlerts.length} élément{allAlerts.length > 1 ? 's' : ''})</div>
      ...
      {/* Filaments */}
      <div>
        <div className="flex items-center gap-2 mb-3"><Layers size={16} style={{ color: '#8B5CF6' }} /><span className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Filaments</span></div>
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
          {filamentCards.map((s, i) => (
            <motion.div key={s.label} custom={i} initial="hidden" animate="visible" variants={cardVariants}>
              <div className="glass-card-purple p-4 relative overflow-hidden hover:scale-[1.02] transition-transform duration-200" style={{ boxShadow: `0 0 15px ${s.color}22` }}>
      ...
      {/* Fabrication */}
      <div>
        <div className="flex items-center gap-2 mb-3"><Cpu size={16} style={{ color: '#00D9FF' }} /><span className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Fabrication</span></div>
            ...
            return (
              <motion.div key={s.label} custom={i + 4} initial="hidden" animate="visible" variants={cardVariants}>
                <div className="glass-card p-5 relative overflow-hidden hover:scale-[1.02] transition-transform duration-200">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-4" style={{ background: `${s.color}18`, border: `1px solid ${s.color}40` }}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <motion.div className="xl:col-span-2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <div className="glass-card p-5 h-72">
            <div className="flex items-center gap-2 mb-4"><BarChart3 size={16} style={{ color: '#00D9FF' }} /><span className="text-sm font-semibold text-slate-300">Dernières productions</span></div>
            {chartData.length === 0 ? (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <div className="glass-card p-5 h-72">
            <div className="flex items-center gap-2 mb-4"><Layers size={16} style={{ color: '#8B5CF6' }} /><span className="text-sm font-semibold text-slate-300">Stock par matériau</span></div>
            {matPie.length === 0 ? <div className="h-48 flex items-center justify-center text-slate-600 text-sm">Aucune bobine</div> : (
              <>
                <ResponsiveContainer width="100%" height="62%">
                  <PieChart>
                    <Pie data={matPie} cx="50%" cy="50%" innerRadius={40} outerRadius={68} paddingAngle={3} dataKey="value">
                      {matPie.map(entry => <Cell key={entry.name} fill={MATERIAL_COLORS[entry.name as keyof typeof MATERIAL_COLORS] ?? '#94a3b8'} stroke="transparent" />)}
                    </Pie>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}>
          <div className="glass-card p-5">
            <div className="flex items-center gap-2 mb-4"><Thermometer size={16} style={{ color: '#FF8C00' }} /><span className="text-sm font-semibold text-slate-300">Dernières impressions</span></div>
            {spoolHistory.length === 0 ? <div className="py-6 text-center text-slate-600 text-sm">Aucune impression enregistrée</div> : (
              [...spoolHistory].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5).map((h, i) => (
                <div key={h.id ?? i} className="table-row flex items-center gap-3 py-2.5 px-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500">{h.brand} {h.material}</div>
                  </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <div className="glass-card p-5">
            <div className="flex items-center gap-2 mb-4"><Cpu size={16} style={{ color: '#00D9FF' }} /><span className="text-sm font-semibold text-slate-300">Dernières productions</span></div>
            {productionHistory.length === 0 ? <div className="py-6 text-center text-slate-600 text-sm">Aucune production enregistrée</div> : (
              productionHistory.slice(0, 5).map((h, i) => (
                <div key={h.id ?? i} className="table-row flex items-center gap-3 py-2.5 px-1">
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="badge" style={{ background: 'rgba(0,255,136,0.1)', color: '#00FF88', border: '1px solid rgba(0,255,136,0.2)' }}>{fmt(h.marginPercent)}%</div>
                </div>
              ))
            )}
          </div>
